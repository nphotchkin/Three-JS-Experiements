...
class VectorMapper {
    ...
    static toYukaVector(threeVector3: THREE.Path, z: number): YUKA.Vector3[] {
        ...
        var points = threeVector3.getPoints();
...
function init() {
    ...
    spawnSphereAtPosition(BLACK_HOLE.position);
    ...
    controls.target.copy( BLACK_HOLE.position );
    controls.update();
...
var quadraticPaths = [
    aQuadraticCurveConvergingToCenter(-30, 5, 0, 5),
    aQuadraticCurveConvergingToCenter(-25, 5, 0, 5),
...
var pathingAgents: PathFollowingAgent[] = [];

quadraticPaths.forEach(path => {
    var yukaPaths = VectorMapper.toYukaVector(path, 0);

    pathingAgents.push(
        new PathFollowingAgent({
...
function spawnSphereAtPosition(position: THREE.Vector3) {
    ...
    var color = new THREE.Color(102, 56, 219)
    const matSphere = new THREE.MeshStandardMaterial( { color: color, roughness: 0, metalness: 0 } );
    matSphere.transparent = true;
    matSphere.opacity = 1; // fully visible
    
    meshKnot = new THREE.Mesh(sphereGeom, matSphere);
...
function addBackDrop() {
    ...
    var rec1Color = new THREE.Color(87, 52, 178)
    const rectLight1 = new THREE.RectAreaLight( rec1Color, 0.2, 4, 9);
    rectLight1.position.set( - 2, 0, 5 );
    scene.add( rectLight1 );

    const rectLight2 = new THREE.RectAreaLight( 0x00ff00, 25, 4, 9);
    rectLight2.position.set( 0, 0, 5 );
    scene.add( rectLight2 );

    const rectLight3 = new THREE.RectAreaLight( 0x0000ff, 20, 4, 9);
...
function animation() {
    ...
    pathingAgents.forEach(pathingAgent => {
